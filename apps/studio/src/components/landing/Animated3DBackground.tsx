...
function FloatingParticles({ mouse, introProgress, isEasterEggActive, easterEggProgress }: { mouse: { x: number; y: number }; introProgress: number; isEasterEggActive: boolean; easterEggProgress: number }) {
  ...
  useFrame((state) => {
    ...
    // Smooth transition for easter egg
    const easterEggIntensity = isEasterEggActive ? Math.sin(easterEggProgress * Math.PI) : 0;

    materialRef.current.opacity = (0.4 + easterEggIntensity * 0.4) * introScale;
    materialRef.current.size = 0.08 + easterEggIntensity * 0.07;
    ...
    for (let i = 0; i < posArray.length; i += 3) {
      ...
      // Easter egg excitement - smooth transition
      const excitement = easterEggIntensity * 0.5;
      const speedMult = 1 + easterEggIntensity * 0.5;
...
function AnimatedMesh({
  ...
  useFrame((state, delta) => {
    ...
    // Base appearance
    const baseOpacity = (0.25 + pulse * 0.1) * introScale;
    const baseEmissive = (0.4 + Math.sin(time * 1.2) * 0.2) * introScale;
    ...
    if (isEasterEggActive) {
      ...
      materialRef.current.opacity = baseOpacity + peak * 0.5;
      materialRef.current.emissiveIntensity = baseEmissive + peak * 1.5; // Brighter but not blinding
      ...
      // Use progress to drive rotation speed for smooth start/end
      const rotationSpeedMult = 1 + peak * 3;
      meshRef.current.rotation.x += delta * rotationSpeedMult;
      meshRef.current.rotation.y += delta * rotationSpeedMult * 1.5;

      // Update rotationRef to match current rotation so there's no jump when returning
      rotationRef.current.x = meshRef.current.rotation.x;
      rotationRef.current.y = meshRef.current.rotation.y;

      // Gentle pulse scale - single cycle based on progress
      const wobble = Math.sin(easterEggProgress * Math.PI * 2) * 0.2;
      ...
      materialRef.current.emissiveIntensity = baseEmissive;
    }
  ...
  useEffect(() => {
    if (!isEasterEggActive) {
      ...
      // Sync rotation ref to current mesh rotation to prevent snap
      if (meshRef.current) {
        rotationRef.current.x = meshRef.current.rotation.x;
        rotationRef.current.y = meshRef.current.rotation.y;
      }
...
function OrbitingSphere({
  ...
  useFrame((state) => {
    ...
    if (isEasterEggActive) {
      materialRef.current.opacity = 0.4 + easterEggIntensity * 0.4;
      materialRef.current.emissiveIntensity = 0.6 + easterEggIntensity * 0.4 + Math.sin(time * 2) * 0.3;
      ...
      // Interpolate between normal and wild positions based on intensity
      const normalR = 2;
      const normalr = 0.5;
      ...
      const normalX = (normalR + normalr * Math.cos(v)) * Math.cos(u);
      const normalY = (normalR + normalr * Math.cos(v)) * Math.sin(u);
      const normalZ = normalr * Math.sin(v);

      const wildX = Math.cos(wildAngle) * wildRadius;
      const wildY = Math.sin(wildAngle * 1.5) * wildRadius * 0.5;
      const wildZ = Math.sin(wildAngle) * wildRadius;

      sphereRef.current.position.x = THREE.MathUtils.lerp(normalX, wildX, easterEggIntensity);
      sphereRef.current.position.y = THREE.MathUtils.lerp(normalY, wildY, easterEggIntensity);
      sphereRef.current.position.z = THREE.MathUtils.lerp(normalZ, wildZ, easterEggIntensity);

      sphereRef.current.scale.setScalar(introScale * (1 + pulse * (1 + easterEggIntensity)));
    } else {
...
function OrbitingSpheres({ mouse, introProgress, isEasterEggActive, easterEggProgress }: { mouse: { x: number; y: number }; introProgress: number; isEasterEggActive: boolean; easterEggProgress: number }) {
  ...
  useFrame((state, delta) => {
    ...
    const normalSpeed = baseSpeed + mouseInfluence + speedVariation;

    // Smoothly interpolate speed
    orbitSpeedRef.current = THREE.MathUtils.lerp(normalSpeed, 1.5, easterEggIntensity);
...
function EasterEggEffects({ isActive, progress }: { isActive: boolean; progress: number }) {
  ...
  useFrame((state) => {
    ...
    const radius = 5 + intensity * 1; // Expand radius slightly

    // Orbit around slowly
    const targetX = Math.sin(time * orbitSpeed) * radius;
    const targetZ = Math.cos(time * orbitSpeed) * radius;
    const targetY = Math.sin(time * 0.2) * 1.5 * intensity;
  ...
  return (
    <group>
      ...
      <pointLight position={[0, 0, 0]} intensity={1.5 * Math.sin(progress * Math.PI)} color="#ffffff" distance={10} />
...
export function Animated3DBackground({
  ...
  const handleSpinDetected = () => {
    const now = Date.now();
    if (!isEasterEggActive && now - lastEasterEggTimeRef.current > cooldownDuration) {
  ...
  useEffect(() => {
    ...
    const animateEasterEgg = () => {
      ...
      if (progress < 1) {
        ...
        lastEasterEggTimeRef.current = Date.now();
  ...
  return (
    <div className={`fixed inset-0 z-0 pointer-events-none w-screen h-screen transition-opacity duration-1000 ease-out ${
